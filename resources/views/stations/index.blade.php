...
<div class="max-w-4xl mx-auto px-4 py-6">
    ...
    <form method="GET" action="{{ route('stations.index') }}" class="bg-white p-4 rounded-xl border border-slate-200 mb-6 shadow-sm space-y-4">
        <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
            <div>
                <label class="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1">Carburante</label>
                <select name="fuel_type" onchange="this.form.submit()" class="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 text-sm font-medium focus:ring-2 focus:ring-blue-500">
            ...
            <div>
                <label class="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1">Comune</label>
                <select name="municipality" onchange="this.form.submit()" class="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 text-sm font-medium focus:ring-2 focus:ring-blue-500">
            ...
            <div class="col-span-2 md:col-span-1">
                <label class="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1">Erogazione</label>
    ...
    <h2 class="text-lg font-bold text-slate-800 mb-4">Distributori a Rovigo ({{ $stations->count() }})</h2>
    ...
    <!-- Mappa Globale -->
    <div class="mt-8">
        <x-map-view :stations="$stations->toArray()" />
    </div>
